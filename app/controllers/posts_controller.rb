class PostsController < ApplicationController
  ...
  def index
    ...
    @posts=Post.left_joins(:tickets).group(:content).select("posts.content,posts.id AS id,COUNT(`tickets`.`id`) AS tickets_count")
    ...
    @posts_new=Post.all.order(created_at: :desc).limit(2)
  end
